...
//        waypointSignalID = registerSignal("wayPointID");
...
//        emit(waypointSignalID, this->wayPointID);
...
void HICADroneProtocol::handleTelemetry(
...
//    this->checkMobility();
    ...
    if (payload != nullptr) {
        ...
        std::cout << "[ATUAL: " << std::ctime(&end_time) << "] \n\tDRONES::"
                << this->getdronesIDsList() << endl;
        std::cout << "\tOXEN #" << this->getOxIDs().size() << "::"
                << this->getOxIDsList() << endl;
        std::cout << "----------------------------------------------------\n\n"
                << endl;

        if (this->getOxIDs().size() > 2
                && this->getOxIDs().size()
                        == (int) this->getParentModule()->getParentModule()->par(
                                "numSensors")) {
            std::cout << "total: "
                    << (int) this->getParentModule()->getParentModule()->par(
                            "numSensors") << endl;
...
void HICADroneProtocol::goToTarget(int target){
...
//        Coord coord = coordinateSystem->computeSceneCoordinate(tour[wayPointID]);
//
//        MobilityCommand *command = new MobilityCommand();
...
void HICADroneProtocol::printDroneInformation(){
    auto end = std::chrono::system_clock::now();
                std::time_t end_time = std::chrono::system_clock::to_time_t(end);
                std::cout << "[ATUAL: "<< std::ctime(&end_time) <<"]\tDRONES::" << this->getdronesIDsList()<< endl;
                std::cout << "\tOXEN #" << this->getOxIDs().size() << "::"
                        << this->getOxIDsList() << endl;
                std::cout
                        << "----------------------------------------------------\n\n"
